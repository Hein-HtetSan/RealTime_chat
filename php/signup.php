<?php
    ...
    if(!empty($fname) && !empty($lname) && !empty($email) && !empty($password)){
        ...
        if(filter_var($email, FILTER_VALIDATE_EMAIL)){ // if email is vaild
            ...
            if(mysqli_num_rows($sql)){ // if email already exist
                ...
                if(isset($_FILES['image'])){ // if file is uploaded
                    ...
                    $extensions = ['png', 'jpeg', 'jpg']; // thse are some valid img ext and we've store them in array
                    if(in_array($img_ext, $extensions) === true){ // if user uploaded img ext is matched with an array extensions
                        $time = time(); //this will return us current time;
                                        //we need this time becacuse when you uploading user img to our folder we rename user file with current file
                                        //so all the image file will have a unique name;
                        //let's move the user uploaded image to our particular folder
                        $new_img_name = $time.$img_name;
                        if(move_uploaded_file($tmp_name, "../images/".$new_img_name)){
                            $status = "Active now"; // once use singed up then his stuts will be active now
                            ...
                            if($sql2){// if these data inserted
                                ...
                                if(mysqli_num_rows($sql3) > 0){
                                    ...
                                    $_SESSION['unique_id'] = $row['unique_id']; // using this session we used user unique_id in othe php file
                                    echo "success";
                                }
                            }else{
                        ...
                        echo "Please select the valid file - jpeg, jpg, png!";
                    }
